...
// Pindahkan loadPelanggan ke scope global
    ...
    $.get('/konveksi/api/pelanggan.php?action=list&q=' + encodeURIComponent(keyword), function(data) {
        ...
        data.forEach((p, i) => {
            ...
            tr.html(`
                ...
                <td>
                    <span class="badge bg-primary btn-trx" style="cursor:pointer"
                        data-id="${p.id_pelanggan}" data-name="${p.name}">
                ...
                <td>
                    ...
                    <button class="btn btn-sm btn-info btn-ukuran"
                        data-id="${p.id_pelanggan}"
                        data-name="${p.name}">
...
$(document).ready(function() {
    $('#searchPelanggan').val('');
    setTimeout(() => loadPelanggan(), 100);

    // Event delegation untuk tombol di tabel
    $(document).on('click', '.btn-edit', function() {
        let d = $(this).data();
        bukaEdit(d.id, d.name, d.email, d.nohp, d.alamat);
    });

    $(document).on('click', '.btn-ukuran', function() {
        let d = $(this).data();
        bukaUkuran(d.id, d.name);
    });

    $(document).on('click', '.btn-hapus', function() {
        let d = $(this).data();
        hapusPelanggan(d.id, d.name);
    });

    $(document).on('click', '.btn-trx', function() {
        let d = $(this).data();
        lihatTransaksi(d.id, d.name);
    });
...
function bukaEdit(id, name, email, no_hp, alamat) {
    ...
    $('#edit_password').val('');
    let elEdit = document.getElementById('modalEdit');
    let instEdit = bootstrap.Modal.getInstance(elEdit);
    if (instEdit) instEdit.dispose();
...
function bukaUkuran(id_pelanggan, nama) {
    ...
    let elModal = document.getElementById('modalUkuran');
    let instance = bootstrap.Modal.getInstance(elModal);
...
function lihatTransaksi(id, name) {
    ...
    let elTrx = document.getElementById('modalDetailTrx');
    let instTrx = bootstrap.Modal.getInstance(elTrx);
    if (instTrx) instTrx.dispose();
    new bootstrap.Modal(elTrx).show();
